...
export declare const createAppSlice: <T>() => import('@reduxjs/toolkit').Slice<
  AppState<T>,
  {
    runNonBlockingAction: (
      state: import('immer/dist/internal').WritableDraft<AppState<T>>,
      action: PayloadAction<string>,
    ) => {
      requiredNonBlockingActions: string[];
      pendingNonBlockingAction: string;
      pathKey: import('immer/dist/internal').WritableDraft<PathKey<T>>;
      visitedPathKeys: import('immer/dist/internal').WritableDraft<
        PathKey<T>
      >[];
      isLoading: boolean;
      finishedNonBlockingActions: string[];
